  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['course:teach:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['course:teach:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="最大容量" prop="max">
          <el-input v-model="form.max" placeholder="请输入最大容量" />
        </el-form-item>
      </el-form>
...
<script>
import { listTeach, getTeach, delTeach, addTeach, updateTeach } from "@/api/course/teach";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listTeach(this.queryParams).then(response => {
        this.teachList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateTeach(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addTeach(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除教授课程编号为"' + ids + '"的数据项？').then(function() {
        return delTeach(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('course/teach/export', {
        ...this.queryParams
      }, `teach_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
